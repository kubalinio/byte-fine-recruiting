...
const authorizeHandler = http.post<
  LoginMutationArguments,
  never,
  LoginMutationResponse
>("/authorize", async () =>
...
const meHandler = http.get<PathParams, DefaultBodyType, GetMeQueryResponse>(
  "/me",
  async () =>
    HttpResponse.json(
      {
...
const createUsers = (numUsers = 40) => {
  return Array.from({ length: numUsers }, (el, index) => ({
    id: `${index}`,
    name: `User ${index + 1}`,
  }));
};

const usersHandler = http.get<PathParams, DefaultBodyType, GetUsersResponse>(
  "/users",
  ({ request }) => {
    ...
    return HttpResponse.json(
      { users: paginatedUsers, nextPage: nextPageCursor },
      { status: 200 }
    );
  }
);
